...
<div class="col-lg-12 mx-auto">
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="nav-item">
            <a class="nav-link" href="{{ route('introductions.site',$section_id) }}">資源網站</a>
        </li>
        @foreach($section_pages as $s_p)
            <li class="nav-item">
                <?php 
                    $active = ($s_p->id == $section_page->id)?"active":null;
                ?>
                <a class="nav-link {{ $active }}" href="{{ route('introductions.section_page',$s_p->id) }}">{{ $s_p->title }}</a>
            </li>
        @endforeach
        <li class="nav-item">
    ...
    <br>
    <form action="{{ route('introductions.section_page_update',$section_page->id) }}" method="post" id="update_form" onsubmit="return false">
        @csrf
        ...
        <div class="form-group my-2">
            <textarea id="mytextarea" name="content" required>{{ $section_page->content }}</textarea>            
        </div>
        
        <input type="hidden" name="section_id" value="{{ $section_id }}">        
        <div class="form-group">
            <a href="#!" class="btn btn-success btn-sm" onclick="sw_confirm2('確定儲存？','update_form')">儲存設定</a>
        </div>
    </form>       
    <br>    
</div>
