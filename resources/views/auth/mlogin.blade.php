    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <ul class="nav nav-tabs">
                <li class="nav-item">
                  <a class="nav-link" aria-current="page" href="{{ route('glogin') }}">GSuite 登入</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link" href="#">雲端帳號登入</a>
                </li>
                <li class="nav-item">
                  <a class="nav-link active" href="{{ route('mlogin') }}">本機登入</a>
                </li>
            </ul>
            @if((session('login_error')) < 3)
                    <form action="{{ route('mauth') }}" method="post" id="login_form">
                        @csrf
                        <label for="username" class="form-label">本機帳號</label>
